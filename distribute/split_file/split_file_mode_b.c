...
static int qsort_cmpfunc(const void *a, const void *b)
{
    ...
    int cmp = strcmp(y->dst_path, x->dst_path);
    if(cmp==0)
...
void split_file_mode_b(FileSource *Parent, FileSourceProperties *ParentCount, FileSource **Child, FileSourceProperties *ChildCount, int ChildNum)
{
    int i;
